import{c as a}from"./index-DmiQPPFU.js";import"./debug-vendor-DYGKXP-V.js";import"./three-vendor-BEBCUPdy.js";import"./motion-vendor-DjKJqAo0.js";import"./three-examples-vendor-DV7Fnw5Q.js";import"./physics-vendor-xJPXo_LE.js";import"./audio-vendor-CZjnqYNJ.js";class w{constructor(){this.$toggle=document.querySelector(".js-resume-toggle"),this.$overlay=document.querySelector(".js-resume-overlay"),this.$nav=document.querySelector(".rm-nav"),this.$content=document.querySelector(".rm-content"),this.isActive=!1,this.$previousFocus=null,this.$backgroundSurfaces=Array.from(document.querySelectorAll(".nav-dock, .contact-icons, .controls-panel")),this.handleKeydown=this.handleKeydown.bind(this),!(!this.$toggle||!this.$overlay||!this.$nav||!this.$content)&&(this.render(),this.prepareStagger(),this.buildSnowLayer(),this.buildProgressRail(),this.$closeButtons=document.querySelectorAll(".js-resume-close"),this.$sections=document.querySelectorAll(".rm-section"),this.$navLinks=document.querySelectorAll(".rm-nav a"),this.bindToggle(),this.bindSkipLink(),this.bindScrollReveal(),this.bindNavHighlight(),this.bindScrollProgress(),window.__resumeModeRequested&&(window.__resumeModeRequested=!1,this.open({focusTarget:"heading"})))}render(){this.$nav.innerHTML=this.renderNav(),this.$content.innerHTML=this.renderAbout()+this.renderSkills()+this.renderProjects()+this.renderActivities()+this.renderExperience()+this.renderCertifications()+this.renderContact()+this.renderFooter()}prepareStagger(){document.querySelectorAll(".rm-skills-group").forEach(t=>{t.querySelectorAll(".rm-skills-tags span").forEach((e,s)=>{e.style.setProperty("--stagger-index",s)})}),document.querySelectorAll(".rm-section").forEach(t=>{t.querySelectorAll(".rm-timeline-item, .rm-project, .rm-cert-card").forEach((e,s)=>{e.style.setProperty("--stagger-index",s)})})}buildSnowLayer(){this.$snowCanvas=document.createElement("canvas"),this.$snowCanvas.className="rm-snow-layer",this.$overlay.insertBefore(this.$snowCanvas,this.$overlay.firstChild),this.snowContext=this.$snowCanvas.getContext("2d"),this.snowParticles=[],this.snowParticleCount=70,this.snowAnimationId=null;const t=()=>{this.$snowCanvas.width=window.innerWidth,this.$snowCanvas.height=window.innerHeight};t(),window.addEventListener("resize",t);for(let e=0;e<this.snowParticleCount;e++)this.snowParticles.push({x:Math.random()*window.innerWidth,y:Math.random()*window.innerHeight,radius:.6+Math.random()*1.8,speed:.15+Math.random()*.4,drift:Math.random()*.4-.2})}startSnowAnimation(){if(this.snowAnimationId)return;const t=()=>{const e=this.snowContext;e.clearRect(0,0,this.$snowCanvas.width,this.$snowCanvas.height),e.fillStyle="rgba(255, 255, 255, 0.7)",this.snowParticles.forEach(s=>{s.y+=s.speed,s.x+=s.drift,s.y>this.$snowCanvas.height&&(s.y=-4,s.x=Math.random()*this.$snowCanvas.width),e.beginPath(),e.arc(s.x,s.y,s.radius,0,Math.PI*2),e.fill()}),this.snowAnimationId=window.requestAnimationFrame(t)};t()}stopSnowAnimation(){this.snowAnimationId&&(window.cancelAnimationFrame(this.snowAnimationId),this.snowAnimationId=null)}buildProgressRail(){this.$progressRail=document.createElement("div"),this.$progressRail.className="rm-progress-rail",this.$progressRail.innerHTML='<div class="rm-progress-rail-fill"></div>',this.$overlay.appendChild(this.$progressRail),this.$progressFill=this.$progressRail.querySelector(".rm-progress-rail-fill")}bindScrollProgress(){this.$overlay.addEventListener("scroll",()=>{const t=this.$overlay.scrollHeight-this.$overlay.clientHeight,e=t>0?this.$overlay.scrollTop/t:0;this.$progressFill.style.height=`${Math.min(100,Math.max(0,e*100))}%`})}renderNav(){return`
        ...
        `}renderSkills(){return`
            <section class="rm-section" id="rm-skills">
                ...
                <div class="rm-skills-grid">
                    ${a.skills.map(e=>{const s=e.tags.map(r=>`<span class="interactive-hover interactive-hover--opacity">${r.name||r}</span>`).join("");return`
    ...
    `;return`data:image/svg+xml;charset=UTF-8,${encodeURIComponent(i)}`}statusDotClass(t){const e=(t||"").toLowerCase();return e.includes("progress")||e.includes("soon")?"rm-status-dot--active":e.includes("private")?"rm-status-dot--private":"rm-status-dot--static"}renderProjectCredentials(t=[]){return t.length?`<div class="rm-project-credentials"><span class="rm-project-detail-label">Supporting credentials</span><div class="rm-project-credential-list">${t.map(s=>{const i=`${s.previewImage?`<img src="${s.previewImage}" alt="${s.title} preview">`:'<span class="rm-project-credential-placeholder" aria-hidden="true">DOC</span>'}<span>${s.title}</span>`;return s.href?`<a class="rm-project-credential interactive-hover interactive-hover--opacity" href="${s.href}" target="_blank" rel="noopener">${i}</a>`:`<span class="rm-project-credential is-disabled">${i}</span>`}).join("")}</div></div>`:""}renderProjects(){return`
            <section class="rm-section" id="rm-projects">
                <span class="rm-eyebrow">projects</span>
                <h2>Selected work</h2>
                ${a.projects.map(e=>{const s=e.links||{},r=this.renderProjectLink(s.demo||e.link),i=this.renderProjectLink(s.repository||e.repo),n=this.renderProjectLink(s.file),d=this.statusDotClass(e.status),h=e.technologies||e.tags||[],l=(e.features||[]).map(o=>`<li>${o}</li>`).join(""),c=e.description||e.solution||"";return`
        ...
        `}renderActivities(){return`
            <section class="rm-section" id="rm-activities">
                ...
                ${a.activities.map((e,s)=>{const r=e.link?`<div class="rm-project-links"><a class="rm-project-link interactive-hover interactive-hover--opacity" href="${e.link.href}" target="_blank" rel="noopener">${e.link.text}</a></div>`:"",i=this.statusDotClass(e.status);return`
        ...
        `}renderExperience(){return`
            <section class="rm-section" id="rm-experience">
                ...
                ${a.experience.map(e=>{const s=(e.bullets||[]).map(i=>"<li>"+i+"</li>").join(""),r=e.process||e.benefits?`<div class="rm-experience-details">
              ${e.process?`<div><span>Process</span><p>${e.process}</p></div>`:""}
              ${e.benefits?`<div><span>Benefits</span><p>${e.benefits}</p></div>`:""}
            </div>`:s?`<ul>${s}</ul>`:"";return['<div class="rm-timeline-item">',`<h3>${e.role}</h3>`,`<div class="rm-org">${e.org}</div>`,`<div class="rm-dates">${e.dates}</div>`,r,"</div>"].join(`
`)}).join("")}
            </section>
        `}renderCertifications(){return`
            <section class="rm-section" id="rm-certifications">
                ...
                ${a.certifications.map(e=>{const s=e.items.map(r=>{const i=this.certificatePreviewSrc(r),n=r.credentialUrl||r.previewImage,d=n?`<a class="rm-cert-link" href="${n}" target="_blank" rel="noopener">${r.verificationLabel||(r.credentialUrl?"Verify credential":"Open certificate")}</a>`:'<span class="rm-cert-link is-disabled">Credential unavailable</span>',h=r.credentialId?`<div class="rm-cert-fact"><span>Credential ID</span><p>${r.credentialId}</p></div>`:"",l=n?`<a class="rm-cert-preview-link" href="${n}" target="_blank" rel="noopener" aria-label="Open ${r.title} credential preview"><img class="rm-cert-preview" src="${i}" alt="${r.title} certificate preview"></a>`:`<img class="rm-cert-preview" src="${i}" alt="${r.title} certificate preview">`,c=(r.details||[]).map(o=>`<div class="rm-cert-detail"><span>${o.label}</span><p>${o.text}</p></div>`).join("");return`
              <article class="rm-cert-card">
                  ${l}
                  <div class="rm-cert-copy">
                      <h3>${r.title}</h3>
                      <div class="rm-cert-facts">
                        <div class="rm-cert-fact"><span>Issuer</span><p>${r.issuer}</p></div>
                        <div class="rm-cert-fact"><span>Issued</span><p>${r.issueDate}</p></div>
                        ${h}
                      </div>
                      ${r.description?`<p class="rm-cert-description">${r.description}</p>`:""}
        ...
        `}renderContact(){return`
            <section class="rm-section" id="rm-contact">
                ...
                <div class="rm-contact-links">
                    ${a.contact.map(e=>{const r=e.href.startsWith("http")?' target="_blank" rel="noopener"':"";return`
