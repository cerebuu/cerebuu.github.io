...
export default class Camera {
  ...
  setAngle() {
    ...
    this.angle.items = {
      default: {
        direction: new THREE.Vector3(1.135, -1.45, 1.15),
        zoom: 0.5,
        targetHeight: 0,
      },
      projects: {
        direction: new THREE.Vector3(0.38, -1.4, 1.63),
        zoom: 0.45,
        targetHeight: 4,
      },
      inspect: {
        direction: new THREE.Vector3(0.24, -1.08, 1.28),
        zoom: 0.22,
        targetHeight: 8,
      },
      fpv: {
        direction: new THREE.Vector3(0.08, -0.72, 0.78),
        zoom: 0.12,
        targetHeight: 11.5,
      },
    };
    ...
    this.angle.value.copy(this.angle.items.default.direction);

    // Set method
    this.angle.set = (_name) => {
      const preset = this.angle.items[_name] || this.angle.items.default;

      this.view.current =
        typeof this.angle.items[_name] !== "undefined" ? _name : "default";
      gsap.to(this.angle.value, {
        x: preset.direction.x,
        y: preset.direction.y,
        z: preset.direction.z,
        duration: 2,
        ease: "power1.inOut",
      });
      gsap.to(this.zoom, {
        targetValue: preset.zoom,
        duration: 1.2,
        ease: "power1.inOut",
      });
      gsap.to(this.view, {
    ...
    this.toggleFirstPerson = () => {
      this.angle.set(this.view.current === "fpv" ? "default" : "fpv");
    };
